...
export async function POST(
  ...
  // Get the player achievement with its achievement definition
  ...
  if (!playerAchievement.completed_at) {
    return NextResponse.json(
      { error: "Achievement not yet completed" },
      { status: 400 }
    );
  }

  // Check if already claimed (using claimed_at column)
  ...
  if (achievement.reward_dust > 0) {
    updates.chrono_dust = player.chrono_dust + achievement.reward_dust;
  }

  if (achievement.reward_title && !player.title) {
    updates.title = achievement.reward_title;
  }
